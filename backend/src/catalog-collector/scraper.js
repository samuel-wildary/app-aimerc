...
const MAX_CARREFOUR_ALL_PRODUCT_LIMIT = 100000;
const DEFAULT_CARREFOUR_DETAIL_CONCURRENCY = 6;
const MAX_CARREFOUR_DETAIL_CONCURRENCY = 30;
...
const MAX_PAO_ALL_PRODUCT_LIMIT = 100000;
const DEFAULT_PAO_DETAIL_CONCURRENCY = 8;
const MAX_PAO_DETAIL_CONCURRENCY = 30;
...
const MAX_SAO_LUIZ_PRODUCT_LIMIT = 100000;
const DEFAULT_SAO_LUIZ_CONCURRENCY = 8;
const MAX_SAO_LUIZ_CONCURRENCY = 30;
...
const MAX_PINHEIRO_PRODUCT_LIMIT = 100000;
const DEFAULT_PINHEIRO_CONCURRENCY = 8;
const MAX_PINHEIRO_CONCURRENCY = 30;
...
const MAX_ATACADAO_PRODUCT_LIMIT = 100000;
const DEFAULT_ATACADAO_CONCURRENCY = 6;
const MAX_ATACADAO_CONCURRENCY = 30;

const GBARBOSA_BASE_URL = 'https://www.gbarbosa.com.br';
const GBARBOSA_API_URL = `${GBARBOSA_BASE_URL}/api/catalog_system/pub/products/search`;
const GBARBOSA_PAGE_SIZE = 50;
const DEFAULT_GBARBOSA_PRODUCT_LIMIT = 120;
const MAX_GBARBOSA_PRODUCT_LIMIT = 100000;
const DEFAULT_GBARBOSA_CONCURRENCY = 6;
const MAX_GBARBOSA_CONCURRENCY = 30;

const COMPER_BASE_URL = 'https://www.comper.com.br';
const COMPER_API_URL = `${COMPER_BASE_URL}/api/catalog_system/pub/products/search`;
const COMPER_PAGE_SIZE = 50;
const DEFAULT_COMPER_PRODUCT_LIMIT = 120;
const MAX_COMPER_PRODUCT_LIMIT = 100000;
...
const BRETAS_BASE_URL = 'https://www.bretas.com.br';
const BRETAS_API_URL = `${BRETAS_BASE_URL}/api/catalog_system/pub/products/search`;
const BRETAS_PAGE_SIZE = 50;
const DEFAULT_BRETAS_PRODUCT_LIMIT = 120;
const MAX_BRETAS_PRODUCT_LIMIT = 100000;
...
function gbarbosaCandidates(products) {
  const candidates = [];

  for (const product of products) {
    const productName = normalizeProductName(product.productName || product.productTitle || product.description);
    const productUrl = product.link || (product.linkText ? `${GBARBOSA_BASE_URL}/${product.linkText}/p` : null);
...
async function scrapeGbarbosaProduct(candidate, logCallback) {
  const { ean, imageUrls, productName, productUrl } = candidate;
...
async function scrapeGbarbosaAll(value, logCallback, options = {}) {
  const onProgress = typeof options.onProgress === 'function' ? options.onProgress : () => {};
  const productLimit = clampInteger(value, DEFAULT_GBARBOSA_PRODUCT_LIMIT, 1, MAX_GBARBOSA_PRODUCT_LIMIT);
  const concurrency = clampInteger(options.concurrency, DEFAULT_GBARBOSA_CONCURRENCY, 1, MAX_GBARBOSA_CONCURRENCY);
  const candidates = [];
  const seenEANs = new Set();

  logCallback(`[INFO] Iniciando GBarbosa Completo. Limite: ${productLimit}. Velocidade: ${concurrency} produtos em paralelo.`);
  onProgress({ phase: 'catalog', current: 0, total: productLimit, remaining: productLimit, saved: 0 });

  logCallback(`[INFO] Carregando a arvore de categorias do GBarbosa para contornar limites VTEX...`);
  let leafPaths = [];
  try {
    const treeResponse = await axios.get(`${GBARBOSA_BASE_URL}/api/catalog_system/pub/category/tree/2`, {
      headers: DEFAULT_HEADERS,
      timeout: 20000
    });
    ...
    function getLeafCategoryPaths(categories) {
      ...
      function traverse(item) {
        if (Array.isArray(item.children) && item.children.length > 0) {
          ...
          try {
            const path = new URL(item.url, GBARBOSA_BASE_URL).pathname.replace(/^\/|\/$/g, '');
  ...
  if (leafPaths.length > 0) {
    for (const categoryPath of leafPaths) {
      ...
      for (let from = 0; candidates.length < productLimit; from += GBARBOSA_PAGE_SIZE) {
        if (shouldCancel) break;
        const to = from + GBARBOSA_PAGE_SIZE - 1;

        let products;
        try {
          const response = await axios.get(`${GBARBOSA_BASE_URL}/api/catalog_system/pub/products/search/${categoryPath}`, {
            params: { _from: from, _to: to },
            headers: { ...DEFAULT_HEADERS, Accept: 'application/json', Referer: `${GBARBOSA_BASE_URL}/` },
            timeout: 30000
          });
          products = Array.isArray(response.data) ? response.data : [];
        } catch (error) {
        ...
        for (const candidate of gbarbosaCandidates(products)) {
          if (candidates.length >= productLimit) break;
          if (seenEANs.has(candidate.ean)) continue;
          seenEANs.add(candidate.ean);
          candidates.push(candidate);
        ...
        if (products.length < GBARBOSA_PAGE_SIZE) break;
        await delay(60);
      }
    }
  } else {
    for (let from = 0; candidates.length < productLimit; from += GBARBOSA_PAGE_SIZE) {
      if (shouldCancel) break;
      const to = from + GBARBOSA_PAGE_SIZE - 1;
      logCallback(`[CATALOGO] GBarbosa produtos ${from + 1}-${to + 1} | selecionados: ${candidates.length}/${productLimit}`);

      let products;
      try {
        const response = await axios.get(GBARBOSA_API_URL, {
          params: { _from: from, _to: to },
          headers: { ...DEFAULT_HEADERS, Accept: 'application/json', Referer: `${GBARBOSA_BASE_URL}/` },
          timeout: 30000,
          maxRedirects: 5
        ...
        if (error.response?.status === 416) break;
        throw new Error(`Falha ao consultar o catalogo do GBarbosa: ${error.message}`);
      }
  ...
  const totalSaved = await runWithConcurrency(candidates, concurrency, async candidate => {
    const result = await scrapeGbarbosaProduct(candidate, logCallback);
    completed++;
    saved += result;
    onProgress({
      phase: 'products', current: completed, total: candidates.length,
  ...
  onProgress({ phase: 'complete', current: candidates.length, total: candidates.length, remaining: 0, saved: totalSaved });
  logCallback(`[FIM] GBarbosa finalizado. Produtos consultados: ${candidates.length}. Produtos novos salvos: ${totalSaved}.`);
  return totalSaved;
}

function comperCandidates(products) {
  const candidates = [];

  for (const product of products) {
    const productName = normalizeProductName(product.productName || product.productTitle || product.description);
    const productUrl = product.link || (product.linkText ? `${COMPER_BASE_URL}/${product.linkText}/p` : null);
...
async function scrapeComperProduct(candidate, logCallback) {
  const { ean, imageUrls, productName, productUrl } = candidate;
...
async function scrapeComperAll(value, logCallback, options = {}) {
  const onProgress = typeof options.onProgress === 'function' ? options.onProgress : () => {};
  const productLimit = clampInteger(value, DEFAULT_COMPER_PRODUCT_LIMIT, 1, MAX_COMPER_PRODUCT_LIMIT);
  const concurrency = clampInteger(options.concurrency, DEFAULT_COMPER_CONCURRENCY, 1, MAX_COMPER_CONCURRENCY);
  const candidates = [];
  const seenEANs = new Set();

  logCallback(`[INFO] Iniciando Comper Completo. Limite: ${productLimit}. Velocidade: ${concurrency} produtos em paralelo.`);
  onProgress({ phase: 'catalog', current: 0, total: productLimit, remaining: productLimit, saved: 0 });

  logCallback(`[INFO] Carregando a arvore de categorias do Comper para contornar limites VTEX...`);
  let leafPaths = [];
  try {
    const treeResponse = await axios.get(`${COMPER_BASE_URL}/api/catalog_system/pub/category/tree/2`, {
      headers: DEFAULT_HEADERS,
      timeout: 20000
    });
    ...
    function getLeafCategoryPaths(categories) {
      ...
      function traverse(item) {
        if (Array.isArray(item.children) && item.children.length > 0) {
          ...
          try {
            const path = new URL(item.url, COMPER_BASE_URL).pathname.replace(/^\/|\/$/g, '');
            if (path && !paths.includes(path)) paths.push(path);
          } catch {}
        }
      }
  ...
  if (leafPaths.length > 0) {
    for (const categoryPath of leafPaths) {
      ...
      for (let from = 0; candidates.length < productLimit; from += COMPER_PAGE_SIZE) {
        if (shouldCancel) break;
        const to = from + COMPER_PAGE_SIZE - 1;

        let products;
        try {
          const response = await axios.get(`${COMPER_BASE_URL}/api/catalog_system/pub/products/search/${categoryPath}`, {
            params: { _from: from, _to: to },
            headers: { ...DEFAULT_HEADERS, Accept: 'application/json', Referer: `${COMPER_BASE_URL}/` },
            timeout: 30000
          });
          products = Array.isArray(response.data) ? response.data : [];
        } catch (error) {
        ...
        for (const candidate of comperCandidates(products)) {
          if (candidates.length >= productLimit) break;
          if (seenEANs.has(candidate.ean)) continue;
          seenEANs.add(candidate.ean);
          candidates.push(candidate);
        ...
        if (products.length < COMPER_PAGE_SIZE) break;
        await delay(60);
      }
    }
  } else {
    for (let from = 0; candidates.length < productLimit; from += COMPER_PAGE_SIZE) {
      if (shouldCancel) break;
      const to = from + COMPER_PAGE_SIZE - 1;
      logCallback(`[CATALOGO] Comper produtos ${from + 1}-${to + 1} | selecionados: ${candidates.length}/${productLimit}`);

      let products;
      try {
        const response = await axios.get(COMPER_API_URL, {
          params: { _from: from, _to: to },
          headers: { ...DEFAULT_HEADERS, Accept: 'application/json', Referer: `${COMPER_BASE_URL}/` },
          timeout: 30000,
          maxRedirects: 5
        ...
        if (error.response?.status === 416) break;
        throw new Error(`Falha ao consultar o catalogo do Comper: ${error.message}`);
      }
  ...
  const totalSaved = await runWithConcurrency(candidates, concurrency, async candidate => {
    const result = await scrapeComperProduct(candidate, logCallback);
    completed++;
    saved += result;
    onProgress({
      phase: 'products', current: completed, total: candidates.length,
  ...
  onProgress({ phase: 'complete', current: candidates.length, total: candidates.length, remaining: 0, saved: totalSaved });
  logCallback(`[FIM] Comper finalizado. Produtos consultados: ${candidates.length}. Produtos novos salvos: ${totalSaved}.`);
  return totalSaved;
}
...
function bretasCandidates(products) {
  const candidates = [];

  for (const product of products) {
    const productName = normalizeProductName(product.productName || product.productTitle || product.description);
    const productUrl = product.link || (product.linkText ? `${BRETAS_BASE_URL}/${product.linkText}/p` : null);
...
async function scrapeBretasProduct(candidate, logCallback) {
  const { ean, imageUrls, productName, productUrl } = candidate;
...
async function scrapeBretasAll(value, logCallback, options = {}) {
  const onProgress = typeof options.onProgress === 'function' ? options.onProgress : () => {};
  const productLimit = clampInteger(value, DEFAULT_BRETAS_PRODUCT_LIMIT, 1, MAX_BRETAS_PRODUCT_LIMIT);
  const concurrency = clampInteger(options.concurrency, DEFAULT_BRETAS_CONCURRENCY, 1, MAX_BRETAS_CONCURRENCY);
  const candidates = [];
  const seenEANs = new Set();

  logCallback(`[INFO] Iniciando Bretas Completo. Limite: ${productLimit}. Velocidade: ${concurrency} produtos em paralelo.`);
  onProgress({ phase: 'catalog', current: 0, total: productLimit, remaining: productLimit, saved: 0 });

  logCallback(`[INFO] Carregando a arvore de categorias do Bretas para contornar limites VTEX...`);
  let leafPaths = [];
  try {
    const treeResponse = await axios.get(`${BRETAS_BASE_URL}/api/catalog_system/pub/category/tree/2`, {
      headers: DEFAULT_HEADERS,
      timeout: 20000
    });
    ...
    function getLeafCategoryPaths(categories) {
      ...
      function traverse(item) {
        if (Array.isArray(item.children) && item.children.length > 0) {
          ...
          try {
            const path = new URL(item.url, BRETAS_BASE_URL).pathname.replace(/^\/|\/$/g, '');
            if (path && !paths.includes(path)) paths.push(path);
          } catch {}
        }
      }
  ...
  if (leafPaths.length > 0) {
    for (const categoryPath of leafPaths) {
      ...
      for (let from = 0; candidates.length < productLimit; from += BRETAS_PAGE_SIZE) {
        if (shouldCancel) break;
        const to = from + BRETAS_PAGE_SIZE - 1;

        let products;
        try {
          const response = await axios.get(`${BRETAS_BASE_URL}/api/catalog_system/pub/products/search/${categoryPath}`, {
            params: { _from: from, _to: to },
            headers: { ...DEFAULT_HEADERS, Accept: 'application/json', Referer: `${BRETAS_BASE_URL}/` },
            timeout: 30000
          });
          products = Array.isArray(response.data) ? response.data : [];
        } catch (error) {
        ...
        for (const candidate of bretasCandidates(products)) {
          if (candidates.length >= productLimit) break;
          if (seenEANs.has(candidate.ean)) continue;
          seenEANs.add(candidate.ean);
          candidates.push(candidate);
        ...
        if (products.length < BRETAS_PAGE_SIZE) break;
        await delay(60);
      }
    }
  } else {
    for (let from = 0; candidates.length < productLimit; from += BRETAS_PAGE_SIZE) {
      if (shouldCancel) break;
      const to = from + BRETAS_PAGE_SIZE - 1;
      logCallback(`[CATALOGO] Bretas produtos ${from + 1}-${to + 1} | selecionados: ${candidates.length}/${productLimit}`);

      let products;
      try {
        const response = await axios.get(BRETAS_API_URL, {
          params: { _from: from, _to: to },
          headers: { ...DEFAULT_HEADERS, Accept: 'application/json', Referer: `${BRETAS_BASE_URL}/` },
          timeout: 30000,
          maxRedirects: 5
        ...
        if (error.response?.status === 416) break;
        throw new Error(`Falha ao consultar o catalogo do Bretas: ${error.message}`);
      }
  ...
  const totalSaved = await runWithConcurrency(candidates, concurrency, async candidate => {
    const result = await scrapeBretasProduct(candidate, logCallback);
    completed++;
    saved += result;
    onProgress({
      phase: 'products', current: completed, total: candidates.length,
  ...
  onProgress({ phase: 'complete', current: candidates.length, total: candidates.length, remaining: 0, saved: totalSaved });
  logCallback(`[FIM] Bretas finalizado. Produtos consultados: ${candidates.length}. Produtos novos salvos: ${totalSaved}.`);
  return totalSaved;
}
...
export async function runScraper(options, logCallback) {
  ...
  if (type === 'carrefour_all') {
    ...
    try {
      ...
      if (domain.includes('gbarbosa.com.br')) {
        logCallback(`[INFO] URL do GBarbosa detectada. Usando API VTEX de catalogo em lotes.`);
        return await scrapeGbarbosaAll(value, logCallback, { concurrency, onProgress });
      }
